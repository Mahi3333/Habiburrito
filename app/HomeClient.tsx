...
export interface DisplayItem {
    ...
    price?: string;
    img: string;
    note?: string; // For Chef's Table
    tag?: string;  // For Favorites
}

interface HomeClientProps {
    trendingItems: DisplayItem[];
    showcaseItems: DisplayItem[];
    chefItems: DisplayItem[];
    fanFavorites: DisplayItem[];
}

export default function HomeClient({ trendingItems, showcaseItems, chefItems, fanFavorites }: HomeClientProps) {
    ...
    // Reviews Data (Static for now, could be dynamic later)
    const reviews = [
            ...
            <main>
                        ...
                        <div className="container mx-auto px-6 space-y-0">
                            ...
                            <div className="grid lg:grid-cols-[2fr_1fr] gap-0 overflow-hidden">
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-0">
                                    {showcaseItems.slice(0, 6).map((item, idx) => (
                                        <div key={item.title + idx} className="relative aspect-square overflow-hidden bg-gray-100 group border border-black/5">
                                            <Image src={item.img} alt={item.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                                        </div>
                                    ))}
                                </div>
                                <div className="relative h-full min-h-[320px] bg-gray-100 group border border-black/5 hidden lg:block">
                            ...
                            <div className="grid lg:grid-cols-[1fr_2fr] gap-0 overflow-hidden">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-0">
                                    {showcaseItems.slice(7, 13).map((item, idx) => (
                                        <div key={item.title + idx} className="relative aspect-square overflow-hidden bg-gray-100 group">
                                            <Image src={item.img} alt={item.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                                        </div>
                                    ))}
                                </div>
                ...
                <section ref={favoritesRef} className="py-12 bg-gray-50 border-t border-black/5 relative overflow-hidden">
                    ...
                    <div className="relative w-full overflow-hidden z-10">
                        <div className="absolute inset-y-0 left-0 w-10 sm:w-12 md:w-16 lg:w-32 bg-gradient-to-r from-gray-50/90 to-transparent z-20 hidden sm:block pointer-events-none" />
                        <div className="absolute inset-y-0 right-0 w-10 sm:w-12 md:w-16 lg:w-32 bg-gradient-to-l from-gray-50/90 to-transparent z-20 hidden sm:block pointer-events-none" />
                                        ...
                                        <div key={`${setIndex}-${i}`} className="w-[80vw] md:w-[400px] flex-shrink-0 group cursor-pointer bg-white border border-black/10 rounded-2xl shadow-lg hover:shadow-xl transition-all p-4">
                                            ...
                                            <div className="flex justify-between items-start border-t border-black/10 pt-4">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-brand-black font-medium mt-1">
                                                        {item.price ? item.price : 'Customizable'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
